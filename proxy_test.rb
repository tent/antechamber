# From https://github.com/atmos/camo
# Copyright (c) 2010 Corey Donohoe, Rick Olson under the MIT License
require 'rubygems'
require 'rest_client'
...
module ProxyTests
  ...
  def test_400s_on_urls_without_an_http_host
    assert_raise RestClient::BadRequest do
      request('/picture/Mincemeat/Pimp.jpg')
    end
  end

  def test_400s_on_images_greater_than_5_megabytes
    assert_raise RestClient::BadRequest do
...
class ProxyQueryStringTest < Test::Unit::TestCase
  include ProxyTests

  def request_uri(image_url)
    uri = Addressable::URI.parse(config['host'])
...
class ProxyPathTest < Test::Unit::TestCase
  include ProxyTests
